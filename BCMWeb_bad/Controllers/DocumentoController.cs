        ...
        public ActionResult Index(DocumentosModel model)
        {
            ...
            */
            ...
            if (model.FilterIdProceso > 0)
                model.Documentos = model.Documentos.Where(x => x.Procesos.Where(p => p.IdProceso == model.FilterIdProceso).Count() > 0).ToList().AsQueryable();
            ...
            if (model.FilterProcesoCritico)
                model.Documentos = model.Documentos.Where(x => x.Procesos.Where(p => p.Critico == model.FilterProcesoCritico).Count() > 0).ToList().AsQueryable();
